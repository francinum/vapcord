...
export class WebLogger {
  ...
  createLogger(scopes: Scope | Scope[], trim = 0) {
    return new WebLogger([...this.scopes.slice(-trim), ...[scopes].flat()])
  }
  serialize() {
    return JSON.stringify(this.scopes)
  }
  static deserialize(data: string) {
    return new WebLogger(JSON.parse(data))
  }
  ...
  getScopedArgs() {
    ...
    for (const scope of this.scopes) {
      const cssData = {
        ...
        color:
          scope.color ?? 'hsl(0,calc(var(--saturation-factor, 1)*0%),100%)',
        padding: '0 5px 0 4px',
      ...
      const css =
        [...Object.entries(cssData)].map((pair) => pair.join(':')).join(';') +
          ';' +
          scope.css ?? ''
      args.push(WebLogger.replaceCssVars(css))
    }

    return args
  }

  static replaceCssVars(css: string) {
    return css.replace(
      /var\((--[\w-]+)(?:,([^\)]+))?\)/g,
      (match: string, prop: string, fallback: string) => {
        return (
